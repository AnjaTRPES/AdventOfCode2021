...
class Cave:
    ...
    def nextPathPoss(self, paths, lastCaveName, smallCaveVisitedTwice):
        ...
        if (self.type == 'big') or (self.type == 'start'):
            ...
            for path in paths:
                for neighbour in self.neighbours:
                    if smallCaveVisitedTwice == True:
                        if neighbour.name != lastCaveName and (neighbour.type != 'start'):
                            pathsFoundFromThere = neighbour.nextPathPoss([path+'-'+self.name], self.name, smallCaveVisitedTwice)
                            for p in pathsFoundFromThere:
                                nextPaths.append(p)
                    else:
                        if (neighbour.type != 'start'):
                            pathsFoundFromThere = neighbour.nextPathPoss([path+'-'+self.name], self.name, False)
                            for p in pathsFoundFromThere:
                                nextPaths.append(p)
            return nextPaths
                ...
                for neighbour in self.neighbours:
                    if smallCaveVisitedTwice == True:
                        if (neighbour.type != 'start') and (self.name not in path.split('-')):
                            pathsFoundFromThere = neighbour.nextPathPoss([path+'-'+self.name], self.name, True)
                            for p in pathsFoundFromThere:
                                nextPaths.append(p)
                    else:
                        #print('hm')
                        if (neighbour.type != 'start'):
                            nextVisited = self.name in path.split('-')
                            #print('now visited twice??', nextVisited)
                            pathsFoundFromThere = neighbour.nextPathPoss([path+'-'+self.name], self.name, nextVisited)
                            for p in pathsFoundFromThere:
                                nextPaths.append(p)                
            return nextPaths
...
pathList = caves['start'].nextPathPoss([''], '', False)
pathList.sort()     
